import React, { useEffect, useState } from 'react';
import { authFetch } from './utils/authFetch';
import './pagestyles/CartPage.css';

const CartPage = () => {
  const [cartItems, setCartItems] = useState([]);
  const [modifiedItems, setModifiedItems] = useState({});
  const [loading, setLoading] = useState(true);
  const [totalCost, setTotalCost] = useState(0);

  // Fetch both cart and item details in one go
const fetchCartWithDetails = async () => {
  setLoading(true);
  try {
    // Fetch cart items (already contains all details)
    const cartData = await authFetch('/api/cart');
    if (!cartData || cartData.length === 0) {
      setCartItems([]);
      setTotalCost(0);
      setLoading(false);
      return;
    }
    console.log('Fetched cart items:', cartData);

    // Calculate totals and out-of-stock
    let total = 0;
    const merged = cartData.map((item) => {
      const availableQty = item.available_quantity ?? item.availableQuantity ?? 0;
      const orderedQty = item.quantity;
      const price = item.price || 0;
      const outOfStockQty = Math.max(orderedQty - availableQty, 0);
      const totalPrice = orderedQty * price;

      total += Math.min(orderedQty, availableQty) * price;

      return {
        ...item,
        availableQuantity: availableQty,
        outOfStockQty,
        totalPrice,
      };
    });

    setCartItems(merged);
    setTotalCost(total);
  } catch (err) {
    console.error(err);
    setCartItems([]);
    setTotalCost(0);
  } finally {
    setLoading(false);
  }
};

  useEffect(() => {
    fetchCartWithDetails();
  }, []);

  // Update handleQuantityChange to use id
const handleQuantityChange = (id, newQuantity) => {
  setCartItems((prev) =>
    prev.map((item) => {
      if (item.id === id) {
        const outOfStockQty = Math.max(newQuantity - item.availableQuantity, 0);
        const totalPrice = newQuantity * item.price;
        return { ...item, quantity: newQuantity, outOfStockQty, totalPrice };
      }
      return item;
    })
  );

  setModifiedItems((prev) => ({
    ...prev,
    [id]: newQuantity,
  }));

  const newTotal = cartItems.reduce((acc, item) => {
    const updatedQty = item.id === id ? newQuantity : item.quantity;
    return acc + Math.min(updatedQty, item.availableQuantity) * item.price;
  }, 0);
  setTotalCost(newTotal);
};

  const updateCart = async () => {
    const token = localStorage.getItem('authToken');
    try {
      await Promise.all(
        Object.entries(modifiedItems).map(async ([id, quantity]) => {
          console.log(`Updating item ${id} with quantity ${quantity}`);
          const res = await fetch(
            `https://us-central1-focal-inquiry-468015-q5.cloudfunctions.net/api/api/cart/${id}`,
            {
              method: 'PUT',
              headers: {
                'Content-Type': 'application/json',
                Authorization: `Bearer ${token}`,
              },
              body: JSON.stringify({ quantity }),
            }
          );
          if (!res.ok) throw new Error(`Failed to update item ${id}`);
        })
      );
      alert('Cart updated successfully!');
      setModifiedItems({});
      fetchCartWithDetails();
    } catch (err) {
      console.error(err);
      alert('Failed to update cart');
    }
  };

  const removeItem = async (cartId) => {
  try {
    const token = localStorage.getItem('authToken');
    const res = await fetch(
      `https://us-central1-focal-inquiry-468015-q5.cloudfunctions.net/api/api/cart/${cartId}`,
      {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` },
      }
    );

    if (!res.ok) throw new Error('Failed to delete item');
console.log(`Item ${cartId} removed from cart`, cartItems);
    const updatedCart = cartItems.filter((item) => item.id !== cartId);
    setCartItems(updatedCart);
    setModifiedItems((prev) => {
      const copy = { ...prev };
      delete copy[cartId];
      return copy;
    });

    const newTotal = updatedCart.reduce(
      (sum, item) =>
        sum + Math.min(item.quantity, item.availableQuantity) * item.price,
      0
    );
    setTotalCost(newTotal);
  } catch (err) {
    console.error(err);
    alert('Error removing item from cart');
  }
};

  return (
    <div className="cart-container">
      <h2 className="cart-title">Your Cart</h2>

      {loading ? (
        <p>Loading cart...</p>
      ) : cartItems.length === 0 ? (
        <p>Your cart is empty.</p>
      ) : (
        <>
          <ul className="cart-list">
            {cartItems.map((item) => (
              <li
                key={item.item_id}
                className={`cart-item ${item.availableQuantity === 0 ? 'out-of-stock' : ''}`}
              >
                <div>
                  <div className="item-name">{item.name}</div>
                  <div className="item-model">Model: {item.model}</div>
                  <div className="item-price">Price: ${item.price}</div>
                  <div className="item-qty">
                    Quantity:
          <button
            onClick={() => handleQuantityChange(item.id, item.quantity - 1)}
            disabled={item.quantity <= 1}
          >−</button>
          <span>{item.quantity}</span>
          <button
            onClick={() => handleQuantityChange(item.id, item.quantity + 1)}
            disabled={item.quantity >= item.availableQuantity}
          >+</button>
                  </div>
                  <div className="item-stock">In Stock: {item.availableQuantity}</div>
                  {item.outOfStockQty > 0 && (
                    <div className="item-warning">
                      ⚠ {item.outOfStockQty} item(s) out of stock
                    </div>
                  )}
                  <div className="item-total">Total: ${item.totalPrice.toFixed(2)}</div>
                </div>
                <button className="btn-remove" onClick={() => removeItem(item.id)}>Remove</button>
              </li>
            ))}
          </ul>

          <div className="cart-summary">
            <div className="total-cost">Grand Total: ${totalCost.toFixed(2)}</div>

            {Object.keys(modifiedItems).length > 0 &&
              !cartItems.some((item) => item.outOfStockQty > 0) && (
                <button className="btn-update" onClick={updateCart}>Update Cart</button>
              )}
            {cartItems.some((item) => item.outOfStockQty > 0) && (
              <p className="item-warning">Please adjust your cart — some items exceed current stock.</p>
            )}
          </div>
        </>
      )}
    </div>
  );
};

export default CartPage;